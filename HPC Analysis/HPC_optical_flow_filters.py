    ...
    for index in range(expansion_size, len(arr) - expansion_size):
        ...
        if arr[index] > 0:
            ...
            if index + next_non_zero + expansion_size < len(final_arr):
                final_arr[index - expansion_size:index + next_non_zero + expansion_size] = np.repeat(slice_value,
                                                                                                     2 * expansion_size + next_non_zero)
            else:
...
def combine_close_actions(arr, window):
    ...
    for element in np.where(np.logical_and(1 < non_zero_diff[0], non_zero_diff[0] < window))[0]:
        ...
        final_arr[non_zero_indices[0][element]:non_zero_indices[0][element + 1]] = np.repeat(slice_max,
                                                                                             non_zero_indices[0][element + 1] -
                                                                                             non_zero_indices[0][element])
    return final_arr
