...
if exists("D:\\Rabbit Research Videos\\WP 3.2 Cycle 2\\Analysis\\cage_open_dict.json"):
    with open("D:\\Rabbit Research Videos\\WP 3.2  Cycle 2\\Analysis\\cage_open_dict.json", "r") as f:
...
for dir in sorted(glob.glob('D:\\Rabbit Research Videos\\HPC_Analysis\\WP32_Cycle1\\Action_Diagrams_Intensity_12\\C*')):
    ...
    action_npy_path = os.path.join(
        'D:\\Rabbit Research Videos\\HPC_Analysis\\WP32_Cycle1\\Action_Diagrams_Intensity_12\\', camera_text)
    action_diagram_path = os.path.join(
        'D:\\Rabbit Research Videos\\HPC_Analysis\\WP32_Cycle1\\Action_Diagrams_Intensity_12\\Plots', camera_text)
    ...
    for action_arr in sorted(glob.glob(action_npy_path + '\\*.npy')):
        vid_text = action_arr.rsplit('\\', 1)[1].rsplit('.', 1)[0]
        if ('020000' not in action_arr) and ('filtered' not in action_arr) and not (os.path.exists(os.path.join(action_npy_path, (vid_text + '_filtered.npy')))):
            ...
            # Filtered array
            temp_arr = HPC_optical_flow_filters.window_noise_removal(temp_arr, 3)
