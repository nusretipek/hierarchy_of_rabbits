...
def plot_feeding_series_wp2(arr, camera_text, cage_open_dict, apply_savgol_filter = False, filename = None):
    ...
    ax.plot(x_axis, np.zeros(arr[1].shape[0]) + np.mean(arr[1]), color = 'limegreen', label = 'Mean Feeding Ratio') #Plot mean line
...
for feeding_series in glob.glob('/media/nipek/My Book/Rabbit Research Videos/WP 3.2/Analysis/Feeding_Time_Series/*.npy'):
