...
def plot_optical_flow_hourly_wp2(arr, time_arr, camera_text, cage_open_dict, apply_savgol_filter=False, filename=None):
    ...
    ax.plot(x_axis, np.zeros(arr.shape[0]) + np.mean(arr), color='limegreen',
            label='Mean Action Seconds')  # Plot mean line
...
for dir in sorted(glob.glob('/media/nipek/My Book/Rabbit Research Videos/WP 3.2/C*')):
    ...
    action_analysis_path = os.path.join('/media/nipek/My Book/Rabbit Research Videos/WP 3.2/Analysis/Action_Diagrams',
                                        camera_text)
