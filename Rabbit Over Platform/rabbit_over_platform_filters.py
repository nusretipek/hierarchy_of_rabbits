...
def get_index_slices(arr, number_of_zeroes):
    ...
    index_list = [i for i in range(0, len(arr) - number_of_zeroes) if
                  list(arr[i:i + number_of_zeroes]) != list(zeros_np)]
    ...
    for element in np.where(index_diff > 10)[0]:
        ...
        final_arr[start_point:end_point] = np.repeat(np.median(arr[start_point:index_list[element]]),
    ...
    if 3599 - number_of_zeroes in index_list:
        ...
        final_arr[start_point:index_list[-1]] = np.repeat(np.median(arr[start_point:index_list[-1]]),
                                                          index_list[-1] - start_point)
    return final_arr
...
temp_arr = heat_map_module.numpy_io('read',
                                    '/media/nipek/My Book/Rabbit Research Videos/WP 3.2/Analysis/Rabbit_Over_Platform/Secondly_Sequences_HSV/Camera 8/kon08.20210701_150000.npy')
