...
def window_noise_removal(arr, window_size, spike_height, brightness_param):
...
for dir in sorted(glob.glob('/media/nipek/My Book/Rabbit Research Videos/WP 3.2/C*')):
    ...
    for platform_usage_arr in sorted(glob.glob(rabbit_over_platform_path + '/*.npy')):
        if '020000' not in platform_usage_arr:
            ...
            temp_arr = window_noise_removal(temp_arr, 10, spike_parameters[camera_text][light_parameter + '_Min'],
                                            brightness_parameters[camera_text])
            ...
            heat_map_module.numpy_io('write', os.path.join(rabbit_over_platform_path, (vid_text + '_filtered.npy')),
                                     temp_arr)
